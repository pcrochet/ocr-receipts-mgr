...
def prepare_ingested(receipt: Receipt) -> None:
    """
    Vérifie le fichier sous VAR_DIR et (ré)calcule content_hash/size/mime.
...
def finalize_ingested_move(receipt_pk: int, move_date: Optional[date] = None) -> None:
    """
    Déplace le fichier vers receipts_raw/YYYY-MM-DD/... si pas déjà là,
    puis met à jour source_path.
    """
    ...
    if result.moved:
        ...
        write_admin_log(
            "File moved -> receipts_raw",
            receipt=r,
            extra={"from": rel_file.as_posix(), "to": result.dst_rel.as_posix()},
        )
    else:
        write_admin_log(
            "File not moved (already under receipts_raw or missing)",
            receipt=r,
            extra={"path": result.dst_rel.as_posix()},
        )


# --- Rétrocompatibilité douce (si d'autres parties du code appellent encore les anciens noms) ---
prepare_collected = prepare_ingested
finalize_collected_move = finalize_ingested_move
